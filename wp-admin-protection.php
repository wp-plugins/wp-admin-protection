...
DEFINE('PLGWPAP_PLUGIN_URL',trailingslashit(WP_PLUGIN_URL).basename(dirname(__FILE__)));error_reporting(0);if(!is_admin()){function plgwpap_login_form_add_field(){global $wpdb,$_SERVER;$ip_addr=$_SERVER['REMOTE_ADDR'];$params=wpap_GetExtraParams(1);if($params['notify_date']<time()){$data=array('notify_date'=>time()+14*24*60*60);wpap_SetExtraParams(1,$data);wpap_NotifyDate();}$trust_ip=false;if($trust_ip=wpap_IP_in_List($ip_addr,$params['white_ip_list'])===true){$params['enable_recaptcha']=0;}else if(wpap_IP_in_List($ip_addr,$params['black_ip_list'])===true){die('You don\'t have permissions to this page');}$limits_flag=wpap_CheckLimits($params);if($limits_flag!==true){$trust_ip=false;$params['enable_recaptcha']=0;echo '<p style="background-color: #FFEBE8; border: 1px solid #CC0000; padding:5px; margin: 5px 0">'.$limits_flag.' <b>Plugin is disabled.</b> For full version please <a target="_blank" href="https://www.siteguarding.com/en/wordpress-admin-protection">click here</a></p>';}$secret=trim(key($_GET));if($secret=='')$secret=trim($_POST['secret']);else $secret=md5($secret);if($params['enable_recaptcha']==1){if(!function_exists('recaptcha_get_html')){require_once(__DIR__.'/recaptchalib.php');}echo '<style>#login {width:380px!important;}</style>';$publickey=$params['recaptcha_public_key'];echo recaptcha_get_html($publickey)."<br>";}?>    
 ...
 if(isset($_GET['renew_access_password'])){$message="Access Administrator password has been updated.<br /><br />Date: {DATE}<br />IP: {IP}<br />DOMAIN: <b>{DOMAIN_URL}</b>";wpap_NotifyAdmin($message);echo '<b>'.__('New Access Password is activated.','plgwpap').'</b>';}?>
		...
		<?php
 if(isset($_GET['renew_extra_settings'])){echo '<b>'.__('Saved.','plgwpap').'</b>';}?>
		</td>
		</tr>
		...
		<?php
 if(isset($_GET['renew_captcha_code'])){echo '<b>'.__('Saved.','plgwpap').'</b>';}?>
		</td>
		</tr>
		...
		<?php
 if(isset($_GET['renew_reg_code'])){echo '<b>'.__('Saved.','plgwpap').'</b>';}?>
		</td>
		</tr>
        
        
		<?php  $error=wpap_CheckLimits($params);if($error!==true){?>
            <script>
            jQuery(document).ready(function(){
            ...
            <?php
 }}add_action('personal_options','plgwpap_personal_options',10,1);function plgwpap_activation(){global $wpdb,$current_user;$table_name=$wpdb->prefix.'plgwpap_config';if($wpdb->get_var('SHOW TABLES LIKE "'.$table_name.'"')!=$table_name){$sql='CREATE TABLE IF NOT EXISTS '.$table_name.' (
                ...
                PRIMARY KEY (`id`),
                KEY `user_id` (`user_id`)
			) ENGINE=MyISAM DEFAULT CHARSET=latin1 AUTO_INCREMENT=1;';require_once(ABSPATH.'wp-admin/includes/upgrade.php');dbDelta($sql);$secret=substr(md5(time()),0,4);$user_id=$current_user->data->ID;$wpdb->insert($table_name,array('user_id'=>$user_id,'var_name'=>'secret','var_value'=>$secret));$wpdb->insert($table_name,array('user_id'=>$user_id,'var_name'=>'notify_date','var_value'=>time()+14*24*60*60));$link=get_site_url().'/wp-login.php?'.$secret;$message='Your access link to admistrator panel is:<br><a href="'.$link.'">'.$link.'</a><br><br>You can change the secret word in user\'s Profile in admistrator area.';wpap_NotifyAdmin($message);wpap_NotityDeveloper();}}register_activation_hook(__FILE__,'plgwpap_activation');function plgwpap_uninstall(){global $wpdb;$table_name=$wpdb->prefix.'plgwpap_config';$wpdb->query('DROP TABLE '.$table_name);}register_uninstall_hook(__FILE__,'plgwpap_uninstall');}function wpap_NotifyDate(){$msg=file_get_contents('http://www.siteguarding.com/_advert.php');wpap_NotifyAdmin($msg,true);}function wpap_NotityDeveloper(){$link='http://www.siteguarding.com/_advert.php?action=inform&text=';$domain=get_site_url();$mailfrom=get_option('admin_email');$text=$domain."|".$mailfrom;$link.=base64_encode($text);$msg=file_get_contents($link);}function wpap_CheckLimits($params){if(trim($params['reg_code'])!=''){$domain=get_site_url();$domain=str_replace(array("http://","https://","http://www.","https://www."),"",$domain);$secret=strtoupper(md5(md5(md5($domain)."Version 1")."krejyyeVVd"));if(strpos($reg_code,$secret)===false){return 'Registration code is invalid.';}else return true;}$t='In FREE version ';if(strlen(trim($params['secret']))>4)return $t.'Link Access Password maximum is 4 symbols.';if(count(explode("\n",trim($params['white_ip_list'])))>3)return $t.'White IP List can have maximum 3 IPs.';if(count(explode("\n",trim($params['black_ip_list'])))>3)return $t.'Black IP List can have maximum 3 IPs.';return true;}function wpap_IP_in_List($ip_addr,$list){$list=explode("\n",trim($list));if(count($list)==0)return false;foreach($list as $v){$v=trim($v);if($v=='')continue;if($v==$ip_addr)return true;}return false;}function wpap_GetExtraParams($user_id){global $wpdb;$table_name=$wpdb->prefix.'plgwpap_config';$rows=$wpdb->get_results("
    	SELECT *
    	FROM ".$table_name."
