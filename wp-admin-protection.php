...
if( is_admin() ) {
	...
	function plgwpap_personal_options( $user_profile )
	{
		...
		if( isset( $_GET['renew_access_password'] ) ) {
            ...
            $message = 'Link Secret Word has been updated. Please use the link below to login as administrator:<br /><a href="{LOGIN_LINK}"><b>{LOGIN_LINK}</b></a><br /><br />Date: {DATE}<br />IP: {IP}<br />DOMAIN: <b>{DOMAIN_URL}</b><br />';
            
			$params['secret'] = $secret;
            wpap_NotifyAdmin($message, false, $params);
            
            $domain = get_site_url();
            $tmp['login_link'] = $domain.'/wp-login.php?'.$secret;
			echo '<div class="error below-h2"><p>New Secret Word is activated.<br><br>Please <b>REMEMBER</b> your Link Secret Word and the next time use the link:<br><a href="'.$tmp['login_link'].'"><b>'.$tmp['login_link'].'</b></a></p></div>';
		} ?>
		</td>
		</tr>
...
function wpap_NotifyAdmin($message, $is_advert = false, $data = array())
{
        $domain = get_site_url();
